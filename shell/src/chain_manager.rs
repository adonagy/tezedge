...
#[derive(Clone, Debug)]
// Control message for ask pattern
...
#[actor(CheckChainCompleteness, AskPeersAboutCurrentBranch, DisconnectSilentPeers, GetCurrentHead, NetworkChannelMsg, ShellChannelMsg, SystemEvent)]
...
impl Receive<GetCurrentHead> for ChainManager {
    type Msg = ChainManagerMsg;
    ...
    fn receive(&mut self, ctx: &Context<Self::Msg>, msg: GetCurrentHead, sender: Sender) {
        if let GetCurrentHead::Request = msg {
            if let Some(sender) = sender {
                ...
                if sender.try_tell(GetCurrentHead::Response {
